...
const {extractData, doAsyncTasksSynchronously} = require('./util');
...
class collection extends component {
  ...
  */
  ...
  /**
  * Shallow delete all documents in this collection. Recursively deletes
  * all documents in this collection and their subcollections if recursive flag set in options.
  *
  ...
  */
  delete(options={}){
    ...
    if(options.recursive){
      return this.iterate((doc)=> {
        return doc.ref.getCollections().then((collections)=> {
          if(collections.length == 0){
            return Promise.resolve();
          }
          return doAsyncTasksSynchronously(collections, (cRef)=> {
            return new collection(reference.getPath(cRef)).delete({yes: true, recursive: true});
          });
        });
      });
    }else{
      return this.iterate((doc)=> doc.ref.delete());
    }
  }
  /**
  * BROKEN: throws firebase-tools internal process tick error
  ...
  * will only run when option 'yes' is set to true.
  *
  * Accesses a token that must be set in the functions config or as the environment variable FIREBASE_TOKEN.
  ...
  */
  firebaseToolsDelete(options={}){
    ...
    return firebaseTools.firestore.delete(this._path.toString(), {
        ...
        shallow:  !options.recursive,
        yes: true,
        token: process.env.FIREBASE_TOKEN || functions.config().fb.token
      });
  }
  /**
