let {computeTransactionUpdateObject} from './util';
var document = null;
...
document.node('addDataCountFields', (doc, event, calculation, mapping)=> {
  event((eventData, context)=> doc.instance({eventData, context}, mapping).incrementFields(calculation(data)))));
});
/**
 ...
 */
document.node('addDistributedCountFields', (doc, event, calculation, shards=3, mapping)=> {
  let child = doc.child('shards/{shardId}');
  ...
  child.onCreate((snapshot, context)=> doc.instance({snapshot, context}, mapping).incrementFields(snapshot.data()));
  child.onUpdate((change, context)=>
        doc.instance({snapshot, context}, mapping)
           .incrementFields(computeTransactionUpdateObject(change.after.data(),
              change.before.data(), {subtract: true})));
});
