...
class dataStruct {
  ...
  */
  get parent(){
    if(this._parent){
      ...
      return this.constructor.child(parentPath);
    }
  }
  ...
  */
  extend(path){
    if(typeof path !== 'string'){ throw this.constructor.errors.badPath;}
    if(this.constructor.isRelativePathValid(path.split("/"))){
      return this.constructor(this.path + "/" + path);
    }else {
      return this.constructor.child(this.path + "/" + path);
    }
  }
