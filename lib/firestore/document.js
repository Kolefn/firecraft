...
class document extends component  {
  ...
  * for options supported by Firestore. Pass option { batch: WriteBatch } to add this write operation to a Firestore WriteBatch instead.
  * Set the createOnly option to true to only set data if the document does not yet exist.
  ...
  set(data, options={}){
    ...
    if(options.createOnly){
      return this.transaction((t, doc, ref)=> {
        if(doc.exists){
          return Promise.reject("Document exists & createOnly option set.");
        }else{
  ...
  * @param {Object} setOptions to be passed to the set function
  * @example
  ...
  */
  createOnCreate(docToCreate, mapping, setOptions){
    if(this.path.equalTo(docToCreate)){ throw "Cannot pass the same document to createOnCreate." }
    this.onCreate((snapshot, context)=> docToCreate.instance({snapshot, context}, mapping).set(snapshot.data(), setOptions));
  };
  /**
  ...
  */
  couple(docToCoupleWith, mapping){
    ...
    this.createOnCreate(docToCoupleWith, mapping, {createOnly: true});
    this.deleteOnDelete(docToCoupleWith, mapping);
    docToCoupleWith.createOnCreate(this, invert(mapping), {createOnly: true});
